...
fninit <- function(par,DPSDPE,years,Percent4,Percent5,Repeat5,Repeat6,data,nll=2){
    ...
    if(nll==2){
        nll <--1*sum(log(dlnorm(eps$data$FosterCount[8:(years-1)],log(eps$ages$SpawnerAdult[8:(years-1)]),par[4])))-log(dlnorm(par[3],log(142),(.53)))
        }
...
fninits <- function(par,DPSDPE,years,Percent4,Percent5,Repeat5,Repeat6,data,nll=2){
    ...
    if(nll==2){
        nll <--1*sum(log(dlnorm(eps$data$FosterCount[8:(years-1)],log(eps$ages$SpawnerAdult[8:(years-1)]),par[3])))-log(dlnorm(par[2],log(142),(.53)))
        }
...
fninitdev <- function(par,DPSDPE,years,Percent4,Percent5,Repeat5,Repeat6,data,SDdevI=1,PriorSDI=1,nll=2){
    ...
    if(nll==2){
        ...
        nll <--1*sum(log(dlnorm(eps$data$FosterCount[8:(years-1)],log(eps$ages$SpawnerAdult[8:(years-1)]),par[3])))-log(dlnorm(par[2],log(142),(priorsd)))-sum(log(dnorm(par[4:(years+3)],0,sddev)))
        }
...
print("Yo!")
inputs <- c(.03,120,1,rnorm(38,0,1)) #initializaton
z=optim(par=inputs,fn=fninitdev,method="BFGS",gr=NULL,DPSDPE,years,Percent4,Percent5,Percent6,Repeat6,data,1,1,nll=2,hessian=TRUE,control=c(reltol=10^-132,maxit=35000))
print(z)
...
#sims=simmaker(nsims=10000,years=40,z,sdcon)
...
data$predfemalesmolt_post_dam[1:7] <- data$predfemalesmolt_pre_dam[1:7]*data$passprob[1:7]
data$OceanAdults[1:7] <- data$predfemalesmolt_post_dam[1:7]*data$smoltsurvival[1:7]*marinescale *exp(sims$dev[simindex,1:7])*.74
ages$vir4[1:7] <- Percent4*data$OceanAdults[1:7]
ages$vir5[1:7] <- Percent5*data$OceanAdults[1:7]
ages$vir6[1:7] <- Percent6*data$OceanAdults[1:7]
ages$rep5[1:7]=Repeat6*ages$vir4[1:7]
...
for(i in 8:30)
{
  ...
  data$predfemalesmolt_post_dam[i]=data$predfemalesmolt_pre_dam[i]*data$passprob[i]
  data$OceanAdults[i]=data$predfemalesmolt_post_dam[i]*data$smoltsurvival[i]*marinescale *.74*exp(sims$dev[simindex,i])
  ages$vir4[i]=Percent4*data$OceanAdults[i]
  ages$vir5[i]=Percent5*data$OceanAdults[i]
  ages$vir6[i]=Percent6*data$OceanAdults[i]
  ages$rep5[i]=Repeat6*ages$vir4[i-1]
...
RS <- exp(mean(log(data$OceanAdults[30:(30+5)]/ages$SpawnerAdult[(30):(30+5)])))
abun <- exp(mean(log(ages$SpawnerAdult[30:(55)]),na.rm=TRUE))
...
for( i in 1:ssize)
{
    ...
    RSvec[i]=out$RS
    Abunvec[i]=out$abund
    marineSvec[i] <- mean(out$sim[30:(30+5)])
    QETvec[i] <- ifelse(min(out$QET[30:(30+15)],na.rm=TRUE)>116,0,1)
    confi2[i] <- out$confi
 
}
return(list(conmat=conmat,simmat=simmat,devmat=devmat,QETamat=QETamat,DPEmat=DPEmat,DPSmat=DPSmat,RSmat=RSmat,QETmat=QETmat,RSvec=RSvec,Abunvec=Abunvec,marineSvec=marineSvec,QETvec=QETvec,confi=confi2,DPEvec=as.vector(DPEmat),DPSvec=as.vector(DPSmat)))
}
...
for( i in 1:ssize)
{
    ...
    QETmat[i,] <- ifelse(out$QET[1:years]+out$QET[1:years]>116,0,1)
    RSvec[i]=exp(mean(log((out$data$OceanAdults[(30):(30+5)]+out2$data$OceanAdults[(30):(30+5)])/(out$ages$SpawnerAdult[(30):(30+5)]+out2$ages$SpawnerAdult[(30):(30+5)]))))
    Abunvec[i]=exp(mean(log((out$ages$SpawnerAdult[(30):(55)]+out2$ages$SpawnerAdult[30:55]))))
    marineSvec[i] <- mean((out$sim[30:(30+5)]+out2$sim[30:(30+5)])/2)
    QETvec[i] <- ifelse(min(out$QET[30:(30+15)]+out2$QET[30:(30+15)],na.rm=TRUE)>116,0,1)
...
egg <- function(xx,yy,col1="red",bw=50,title="m",log.vals=FALSE){
...
results <- function(numbersims=100000,morttype=2,years=100)
    {
...
colnames(summaryt) <- c("FOS","A1","A2a","A2b","A3a","A3b","A4","NAA")
        ...
        alpha1=50
plot(NA,NA,xlim=c(0,3), ylim=c(0,5000), ylab="Geo mean Spawners", xlab= "Recruits per Spawner") 
